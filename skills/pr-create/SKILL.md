...
Anything the user supplies wins over what you infer, including their framing of *why* the change exists. Do not second-guess it — supplement it with detail from the code.

## GitHub Access

Use the GitHub MCP server. Resolve its tools with `ToolSearch` (`select:mcp__github__get_me`, plus a keyword search like `github create pull request`) before doing any other work. Names vary by server version — resolve them, do not assume. You will need tools to create a PR, update one, and set labels and reviewers.

If MCP cannot be reached or authenticated, **stop and hand the decision to the user**. Do not diagnose the MCP failure, and do not fall back on your own:
...
Then wait. The `gh` CLI is used **only** on the user explicitly approving it in this session; approval does not carry to a later run.

If the user approves `gh` and it turns out to be missing or unauthenticated, say so in one line, name the command that fixes it (`gh auth login`), and stop. Do not troubleshoot `gh`, try alternative auth paths, or retry — the user asked for a PR, not a CLI debugging session.

## Branch and Changeset

1. `git branch --show-current`. If HEAD is detached, stop and say so. If the working tree has uncommitted changes, say what they are — they will not be in the PR — and ask whether to proceed or commit first.
2. Resolve the base branch, then `git fetch origin <base>` and `git merge-base HEAD origin/<base>`.
3. `git log --oneline <merge-base>..HEAD` and `git diff <merge-base>...HEAD`. If there are no commits ahead of the base, stop — there is nothing to open a PR for.
...
Read `CLAUDE.md` (and what it imports), `README.md`, and any roadmap file before reading the diff. This is what lets the description say how the change fits the project rather than only what it does — a reviewer who already knows the codebase gets nothing from a restatement of the diff.

Then read the changed files for surrounding context, and cluster the changes into the conceptual groups a reviewer would naturally form. On a branch touching more than 50 files, cover the significant groups and summarize the mechanical ones in a line.
...
- **Be scannable in under two minutes.** Drop any section you have nothing real to put in; empty headings cost the reviewer time.
- Link tickets and issues in prose where they belong.

## Confirm, Then Open

Show the user the full description, the title, the base branch, and every option — draft status, labels, reviewers, assignees — as they will be submitted. Ask for confirmation. **Opening a PR notifies people and starts CI, so it happens only on an unambiguous yes.**
...
3. Apply labels, reviewers, and assignees. A label that does not exist in the repo, or a reviewer without access, will be rejected — report which one and leave the PR open without it rather than failing the whole operation.
