...
- **`Agent` calls run in the background by default, so "wait for it" does not block.** A step that needs a sub-agent's result passes `run_in_background: false`; parallel agents whose results are all needed go in one message, each with it.
