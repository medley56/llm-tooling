...
- **The plugin has no `version`, on purpose:** users then update on every commit. The repo ships only skills and agents. No MCP servers: they belong to whatever provisions the machine. No rules: plugins cannot ship them, so prose and conventions go in the skill or agent that needs them, where they load only when that workflow runs. Run `claude plugin validate .` after changing either manifest; the missing-`version` warning is expected.
