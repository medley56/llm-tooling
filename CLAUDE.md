...
- **The GitHub MCP server is named `github` in some repos and `github-mcp` in others**, so the agents list all twelve read tools under both prefixes. A subagent `tools:` entry is a literal tool name; the only patterns it accepts are `mcp__<server>` and `mcp__<server>__*`, which grant a server's *write* tools too. A prefix glob like `mcp__github*` is neither — it matches nothing, and the agent launches with no GitHub tools at all, which looks exactly like an unreachable server.
...
- **A `fallback` must keep the server's name.** `github-mcp`'s fallback swaps OAuth for an `Authorization` header under the same key. A third name would need adding to both agents' `tools:` lists.
