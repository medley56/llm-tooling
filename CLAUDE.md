...
- **Secrets live in the `env` block of `.claude/settings.local.json`** because that path is gitignored in every repo. The install script reads it with `jq` — Claude applies the block to its own sessions but never exports it to a shell, so `${VAR}` is empty there. It substitutes every placeholder before calling the CLI, because Claude Code expands `${VAR}` only for a project-scope `.mcp.json`; a user-scope server keeps the literal string, which surfaces as a 404 against a URL containing `${VAR}`. Installed configs therefore hold cleartext tokens.
