...
- **A `fallback` must keep the server's name.** `github-mcp`'s fallback swaps OAuth for an `Authorization` header, but the tool references are `mcp__github-mcp__*` either way. Renaming a server means moving every `tools:` list and `ToolSearch` selector with it.
